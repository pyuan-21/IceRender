/* This file is automatically generated by "Resources/Shaders/Dissolve/dissolve.main_fs". */
...
void main()
{
	...
	{
		// todo
		// convert position to unit direction
		// in local space, considering there is a bounding-sphere and its center is local origin
		vec3 unitDir = normalize(fPos); 
		float theta = asin(unitDir.y); // return range is [-PI/2, +PI/2]
		float phi;
		if(unitDir.x==0)
		{
			// unitDir is on the y-z plane.
			if(unitDir.z>=0)
				phi = PI/2;
			else
				phi = -PI/2;
			...
			phi = atan(unitDir.z, unitDir.x); // return range is [-PI, PI]

		float u = clamp((theta+PI/2)/PI, 0, 1);
		float v = clamp((phi+PI)/(2*PI), 0, 1);
		float threshold = texture(noiseTex, vec2(u,v)).r;
